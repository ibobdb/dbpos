...
import ModalDetailDiscount from '../views/ModalDiscountDetail';
...
export default function Discount() {
  ...
  const columns = [
    ...
    {
      name: 'Tanggal Mulai',
      selector: row => dateFormat(row.discount_start_on),
    },
    {
      name: 'Tanggal Berakhir',
      selector: row => dateFormat(row.discount_end_on),
    },
    {
      name: 'Perintah',
  ...
  const getProductList = async () => {
    ...
    await productAPI.getProduct(1, 10, productListKeyword).then(response => {
  ...
  const handleSwitchOffDiscount = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        await discountAPI.change_status(id)
          .then(() => {
                ...
                {selectedProduct && selectedProduct.map((item, i) => {
                  return (
                    <li className="list-group-item d-flex justify-content-between align-items-start" key={i}>
                      ...
                      <button className="btn btn-sm btn-danger" type='button' onClick={() => removeSelectedProduct(item.barcode)}>
